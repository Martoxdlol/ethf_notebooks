...
export function ReservationScreen() {
    ...
    const { data: reservation, isPending } = api.getReservation.useQuery(
        reservationId ?? '',
        {
    ...
    const { mutateAsync: deleteReservation } =
        api.deleteReservation.useMutation({
            onError: (error) => {
    ...
    const hours = Math.floor(
        (reservation.to - reservation.from) / 1000 / 60 / 60,
    )
    ...
    return (
        <section className='p-4'>
            <p>
                Pedido por: {reservation.inventoryUserName} (
                {reservation.inventoryUserEmail})
            </p>
            ...
            <div className='max-w-[500px] py-4'>
                ...
                <p className='font-semibold'>
                    de {dayjs(reservation.from).format('HH:mm')} a{' '}
                    {dayjs(reservation.to).format('HH:mm')}
                </p>
